...
import crService from '../../../service/hms/rules/content_rule';
...
export default class extends base {
    ...
    async post() {
        ...
        res.data = await crService.create(data);
        this.json(res);
    }
    ...
    async get() {
        ...
        try {
            if (this.req.params.id) {
                ...
                data = await crService.findOne(map);
                if (data) {
                    rs.data = data
                } else {
                ...
                rs.data.total = await crService.count(map);
                data = await crService.findByQuery(map, paginates, orderby);
    ...
    async delete() {
        ...
        if (this.req.params.id) {
            try {
                res.data = await crService.del({ id: this.req.params.id });
    ...
    async put() {
        ...
        if (this.req.params.id) {
            try {
                res.data = await crService.update({ id: this.req.params.id, isDelete: false }, data);
            } catch (err) {
                res = commConst.getFail();
                res.error.message = err.message;
            }
